...
class MapUI:
    def __init__(self, root, width, height):
        ...
        self.grid = [
            [CellType.PATH for _ in range(self.grid_width)]
    ...
    def create_buttons(self):
        ...
        self.start_button = tk.Button(
            self.button_frame, text="Set Start", command=self.set_start_position
        )
        self.start_button.grid(row=0, column=0)

        self.target_button = tk.Button(
            self.button_frame, text="Set Target", command=self.set_target_position
        )
        self.target_button.grid(row=0, column=1)

        self.obstacle_button = tk.Button(
            self.button_frame, text="Set Obstacles", command=self.add_obstacle
        )
        ...
        self.save_button = tk.Button(
            self.button_frame, text="Save Map", command=self.save_map
        )
        self.save_button.grid(row=0, column=4)

        self.load_button = tk.Button(
            self.button_frame, text="Load Map", command=self.load_map
        )
        self.load_button.grid(row=0, column=5)

        self.path_button = tk.Button(
            self.button_frame, text="Find Path", command=self.find_path
        )
        self.path_button.grid(row=0, column=6)

        self.maze_button = tk.Button(
            self.button_frame, text="Build Maze", command=self.build_maze
        )
        self.maze_button.grid(row=0, column=7)

        self.maze_button = tk.Button(
            self.button_frame, text="Save Path", command=self.save_path
        )
        self.maze_button.grid(row=1, column=2)
    ...
    def save_path(self):
        if not self.start_pos or not self.target_pos or not self.pathfinder:
            messagebox.showwarning(
                "No path found",
                "Please set both start and target positions or find path before saving.",
            )
            return

        path_data = {
            "directions": self.pathfinder.find_directions(),
            "path_coords": self.pathfinder.find_path(),
        }
        ...
        if file_path:
            ...
            messagebox.showinfo("Save Successful", f"Path saved to {file_path}")

    def save_map(self):
        if not self.start_pos or not self.target_pos:
            messagebox.showwarning(
                "Incomplete Map",
                "Please set both start and target positions before saving.",
            )
            return

        # TODO: for PF, we might need like specific coords in the map
        # but for now, grid coordinates suffice
    ...
    def find_path(self):
        if not self.start_pos or not self.target_pos:
            messagebox.showwarning(
                "Incomplete Map",
                "Please set both start and target positions before finding the path.",
            )
            return
        ...
        self.pathfinder = Astar(
            self.grid_height, self.grid_width, self.obstacles, start, dest
        )
    ...
    def build_maze(self):
        ...
        self.grid = [
            [CellType.WALL for _ in range(self.grid_width)]
            for _ in range(self.grid_height)
        ]
        ...
        for _ in range(additional_paths):
            x, y = random.randint(0, self.grid_width - 1), random.randint(
...
if __name__ == "__main__":
    root = tk.Tk()
    app = MapUI(root=root, width=500, height=500)
    root.mainloop()
